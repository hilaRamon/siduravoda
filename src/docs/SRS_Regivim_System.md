...
| `work_number` | number | 1 | ❌ | מספר עבודה פנימי לאותו תלמיד באותו תאריך (1 = ראשון, 2+ = נוסף). לא מוצג בטבלת השיבוצים. |
...
**כלל כפילויות:** ייחודיות ב-DB על `(date, student_id, work_number)`. תלמיד יכול להיות בכמה מקומות באותו יום (עבודה 2, 3, …). דוחות רשימה (ארצנו, דוח תלמיד, PDF יומי) ושכפול יום משתמשים רק ב-`work_number: 1`. טבלת השיבוצים, דוח שעות לפי מקום עבודה ודיווח זמנים כוללים את כל העבודות.
...
**קובץ:** `pages/Assignments.jsx` — `openBulkEdit`, `handleBulkSave`; `lib/assignmentHelpers.js` — `buildBulkAssignmentOps`

**פתיחת הדיאלוג:** בלחיצה על «עריכה מרובה» לא נפתח הדיאלוג אם אותה `student_id` מופיעה יותר מפעם אחת בבחירה (שתי שורות של אותו תלמיד). מוצגת התראה: «סימנת את אותו תלמיד יותר מפעם אחת. בטל את הסימון הכפול ונסה שוב.» זה חל על כל עריכה מרובה, לא רק על פיצול.

**ביצוע רגיל (פיצול עבודה לא מסומן):**
- שדות ריקים → לא משתנים
- מקום עבודה חדש לתלמיד ללא שיבוץ → יוצר שיבוץ חדש (`work_number: 1`) דרך `POST /api/assignments/bulk`
- שורות קיימות מתעדכנות ב-`PUT /api/assignments/bulk` (קריאה אחת)
- אם הבחירה מעורבת (משובצים + ללא שיבוץ) — עד שתי קריאות: POST ואז PUT. אין לולאה לכל שורה.

**פיצול עבודה (checkbox בדיאלוג):**
- השורות שנבחרו לא מתעדכנות
- נוצר שיבוץ חדש לכל תלמיד שנבחר, עם `work_number = max(work_number לאותו תלמיד באותו יום) + 1`, ב-`POST /api/assignments/bulk`
- מקום עבודה חובה (לא «ללא שינוי»)
- שעות: מהדיאלוג, או ברירת מחדל של האפליקציה (4.75) אם השדה ריק. שעות השורה המקורית לא משתנות
- תעריף: מהדיאלוג, או תעריף השורה המקורית אם השדה ריק
- מדלגים: היעדרות מאושרת (שיבוץ למקום אמיתי), תלמידים ללא שיבוץ **ושיבוץ ל-אאא- לפני שיבוץ**, ומקומות עבודה אסורים
- תלמידים ללא שיבוץ / אאא- לפני שיבוץ: התראה עם השמות. אם אין למי לפצל — לא נשלח POST. אם יש גם תלמידים משובצים — `confirmAlert`; אישור מפצל רק אותם, ביטול מבטל
- ה-checkbox והשדות מתאפסים בסגירת הדיאלוג
...
**יצירה:** `POST /api/assignments/bulk`  
**עדכון:** `PUT /api/assignments/bulk`

**POST — קלט:** מערך של רשומות Assignment חדשות (כולל `work_number` לפיצול).

**PUT — קלט:** מערך של `{ id, ...fields }` (שדות לעדכון, כמו `updateAssignment`).

**פעולה:**
- תכנון השורות נשאר ב-client (`buildBulkAssignmentOps`)
- פיצול עבודה → POST אחד
- עריכה מרובה של שורות קיימות → PUT אחד (`bulkWrite`, `ordered: true`)
- תלמידים ללא שיבוץ בעריכה רגילה → POST; בחירה מעורבת → POST ואז PUT
- id חסר / גוף לא-מערך → 400; id לא קיים → 404
