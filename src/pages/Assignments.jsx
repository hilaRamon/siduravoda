      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {new Set(assignments.filter(a => a.workplace_name && !['לא עובד','לימודים','לא יצא'].some(kw => a.workplace_name.includes(kw))).map(a => a.student_id)).size} משובצים מתוך {students.filter(s => s.is_active !== false && (!s.created_date || s.created_date.slice(0, 10) <= date)).length} תלמידים
          </p>
        </div>
